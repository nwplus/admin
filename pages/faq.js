...
import { COLOR, DELETE, EDIT, FAQ, FAQCategory, NEW, VIEW } from '../constants'
import { useAuth } from '../utility/auth'
import { addFaq, deleteFaq, formatDate, getFaqs, getHackathons, getTimestamp, updateFaq } from '../utility/firebase'
...
export default function Faq({ hackathons }) {
  ...
  const handleNew = async () => {
    ...
    setAddNew(false)
    setAlertMsg(`Successfully added the following question: \n${newFaq.question}`)
  }
  ...
  const handleUpdate = () => {
    ...
    setFaqEditing({}, setAlertMsg(`Successfully updated the following question: \n${faqEditing.question}`))
  }
  ...
  const handleDelete = (faqID, confirmed = false) => {
    ...
    setFaqConfirm({}, setAlertMsg(`Successfully deleted the following question: \n${faqs[faqID].question}`))
  }
    ...
    <Page hackathons={hackathons} currentPath="faq">
      <Card>
        ...
        <CardContent style={{ backgroundColor: `${COLOR.BACKGROUND}` }}>
          ...
          >
            <ModalContent page={FAQ} columns={2}>
              ...
              <ModalField
                ...
                modalAction={NEW}
                onChange={event => {
                  handleInput('category', event.target.value, newFaq, setNewFaq)
                }}
          ...
          >
            <ModalContent page={FAQ} columns={2}>
              <ModalField
                ...
                modalAction={EDIT}
                onChange={event => {
                  handleInput('question', event.target.value, faqEditing, setFaqEditing)
                }}
              />
              <ModalField
                dropdown
                ...
                modalAction={EDIT}
                onChange={category => {
                  handleInput('category', category.label, faqEditing, setFaqEditing)
                }}
              />
            </ModalContent>
            <ModalContent page={FAQ} columns={1}>
              <div>
                <ModalField
                  ...
                  modalAction={EDIT}
                  onChange={event => {
                    handleInput('answer', event.target.value, faqEditing, setFaqEditing)
                  }}
                />
                <Label>Hackathons this FAQ applies to</Label>
                {faqEditing.hackathonIDs &&
                  hackathons.map(hackathon => (
                    <Checkbox
                      ...
                      onClick={() => handleToggle(hackathon, faqEditing, setFaqEditing)}
                    />
                  ))}
              </div>
            </ModalContent>
          ...
          >
            <ModalContent page={FAQ} columns={2}>
              <ModalField label="Question" value={faqConfirm.question} modalAction={VIEW} />
              <ModalField label="Category" value={faqConfirm.category} modalAction={VIEW} />
            </ModalContent>
            <ModalContent page={FAQ} columns={1}>
              <div>
                <ModalField label="Answer" value={faqConfirm.answer} modalAction={VIEW} />
                <Label>Hackathons this FAQ applies to</Label>
