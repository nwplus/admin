...
const sort = (arr, type) => {
  switch (type) {
    case SORT.LAST_NAME:
      return arr.sort((a, b) => {
        return a.basicInfo.lastName?.toLocaleLowerCase().localeCompare(b.basicInfo.lastName?.toLocaleLowerCase())
      })
    case SORT.FIRST_NAME:
      return arr.sort((a, b) => {
        return a.basicInfo.firstName?.toLocaleLowerCase().localeCompare(b.basicInfo.firstName?.toLocaleLowerCase())
      })
    case SORT.STATUS:
      // eslint-disable-next-line no-case-declarations
      const priority = Reflect.ownKeys(APPLICATION_STATUS)
      return arr.sort((a, b) => {
        return priority.indexOf(a.status?.applicationStatus) - priority.indexOf(b.status?.applicationStatus)
      })
    case SORT.SCORE:
          ...
          <Page hackathons={hackathons} currentPath="assessment">
            <ToggleBarContainer>
              <ToggleButton
                leftText="Admin"
                rightText="Personal"
                isLeftSelected={isAdminView}
                setIsLeftSelected={setIsAdminView}
              />
              <ToggleButton
                leftText="View All"
                rightText="Assigned Only"
                isLeftSelected={isViewAllApplicants}
                setIsLeftSelected={setIsViewAllApplicants}
              />
            </ToggleBarContainer>
            {isViewAllApplicants ? (
              <span>
                view all applicants place holder <br />{' '}
              </span>
            ) : (
              <span>
                view assigned only place holder <br />{' '}
              </span>
            )}
